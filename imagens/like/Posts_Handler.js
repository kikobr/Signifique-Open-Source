	...
	this.get_first_post = function(){
		$.ajax({
			...
			success: function(data){
				...
				/*
				var html = '<article class="post" data-id="'+post_id+'">';
				html += '<div class="post-destaque-content">';
					html += '<div class="post_img_session"><img class="post_img" src="imagens/icones/small/'+icone+'.png" /></div>';
					html += '<div class="post_content_session"><p class="post_mensagem">'+'<span class="post_nome">'+nome+'</span> '+mensagem+'</p></div>';
				html += "</div>";
				html += '<div class="post-destaque-img"><img src="imagens/star.png" /></div>';
				html += '<div class="like_session"></div>';
				html += '</article>';
				*/				
		...
		$.ajax({
			...
			success: function(data){
				...
				for(var i = 0; i < data.length; i++){
					...
					html += '<div class="like_session"><a class="botao_dislike">N Gostei</a><a class="botao_like" data-count="'+interacoes+'">Gostar</a></div>';
					html += "</article></li>";
